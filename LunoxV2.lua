...
local function AddButton(parent, text, callback)
    ...
    ButtonTemp.Size = UDim2.new(0.209, 0, 0.188, 0)
...
local function AddDropdown(parent, text, options, callback, singleSelect)
    local DropdownTemp = Instance.new("Frame")
    DropdownTemp.Name = "DropdownTemp"
    DropdownTemp.Size = UDim2.new(0.209, 0, 0.188, 0)
        ...
        OptionButton.MouseButton1Click:Connect(function()
            if singleSelect then
                for _, btn in ipairs(OptionsFrame:GetChildren()) do
                    if btn:IsA("TextButton") and btn ~= OptionButton then
